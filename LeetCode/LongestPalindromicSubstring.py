...
class Solution:
    ...
    def longestPalindrome(self, s):
        ...
        i = 0
        palLen = 0
        # Loop invariant: s[(i - palLen):i] is a palindrome.
        ...
        # increments i past sLen - 1 exits the loop early and so skips
        ...
        while i < sLen:
            ...
            for j in range(s, e, -1):
                ...
                # consider the palindrome centered at s[i].
                palLen = 1
                i += 1

        # We know from the loop invariant that len(l) < 2 * sLen + 1, so
        ...
        # Traverse backwards starting from the second-to-last index up
        # until we get l to size 2 * sLen + 1. We can deduce from the
...
def main():
    ins=Solution()
    test(ins.longestPalindrome('ababa'),5)
